...
class HandMeThat():
    def __init__(self):
        ...
        self.xm_Receive_Task1 = StateMachine(
        ...
        with self.xm_Receive_Task1:
            self.xm_Receive_Task1.userdata.degree = -1.6  # 大约0.5是向左25度。1.5是向左75度。具体得导航确定一下
            self.xm_Receive_Task1.userdata.start_sentences = 'I am service robot, what can I do for you'
            self.xm_Receive_Task1.userdata.answer_sentences = 'The object you need is '
            self.xm_Receive_Task1.userdata.remember_sentences = 'i have remember you'
            
            # 目前正在寻找的客人编号-1
            self.xm_Receive_Task1.userdata.present_guest_turn = 0
            # 目前正在寻找的客人坐标
            self.xm_Receive_Task1.userdata.present_guest_pos = Pose()
            ...
            StateMachine.add('SPEAK_GUEST',
                             SpeakText(),
                             transitions={'succeeded': 'RECEIVE_TASKS1',
                                          'aborted': 'SPEAK_GUEST', 'error': 'error'},
                             remapping={'sentences': 'start_sentences'})
            StateMachine.add('RECEIVE_TASKS1',
                             GetTaskH(),
                             transitions={'succeeded': 'SPEAK_OBJECT',
                                          'aborted': 'RECEIVE_TASKS1', 'error': 'error'})
            StateMachine.add('SPEAK_OBJECT',
                             SpeakAnswer2(),
                             transitions={'succeeded1': 'GET_PEOPLE_NAME1','succeeded2': 'GET_PEOPLE_NAME2','succeeded3': 'GET_PEOPLE_NAME3','succeeded4': 'GET_PEOPLE_NAME4',
            ...
            StateMachine.add('GET_PEOPLE_NAME4',
                             FaceRegister4(),
                             transitions={'succeeded': 'SPEAK_REMEMBER',
                                          'aborted': 'GET_PEOPLE_NAME4', 'error': 'error'})
        ...
        #------find the people and get the all task---------#
        self.xm_Receive_Task2 = StateMachine(
            outcomes=['succeeded', 'aborted', 'error'],
            input_keys=["people_pos", 'target', 'action', 'people_correct_pos', 'turn', 'people_name', 'name_pos_dict', 'pos_name_dict'],
            output_keys=["people_pos", 'target', 'action', 'people_correct_pos', 'turn', 'people_name', 'name_pos_dict', 'pos_name_dict'])
        with self.xm_Receive_Task2:
            self.xm_Receive_Task2.userdata.degree = -1.6  # 大约0.5是向左25度。1.5是向左75度。具体得导航确定一下
            self.xm_Receive_Task2.userdata.start_sentences = 'I am service robot, what can I do for you'
            self.xm_Receive_Task2.userdata.answer_sentences = 'The object you need is '
            self.xm_Receive_Task2.userdata.remember_sentences = 'i have remember you'
            
            # 目前正在寻找的客人编号-1
            self.xm_Receive_Task2.userdata.present_guest_turn = 0
            # 目前正在寻找的客人坐标
            self.xm_Receive_Task2.userdata.present_guest_pos = Pose()
            ...
            StateMachine.add('SPEAK_GUEST',
                             SpeakText(),
                             transitions={'succeeded': 'RECEIVE_TASKS2',
                                          'aborted': 'SPEAK_GUEST', 'error': 'error'},
                             remapping={'sentences': 'start_sentences'})
            StateMachine.add('RECEIVE_TASKS2',
                             GetTaskH(),
                             transitions={'succeeded': 'SPEAK_OBJECT',
                                          'aborted': 'RECEIVE_TASKS2', 'error': 'error'})
            StateMachine.add('SPEAK_OBJECT',
                             SpeakAnswer2(),
                             transitions={'succeeded1': 'GET_PEOPLE_NAME1','succeeded2': 'GET_PEOPLE_NAME2','succeeded3': 'GET_PEOPLE_NAME3','succeeded4': 'GET_PEOPLE_NAME4',
                                          'aborted': 'SPEAK_OBJECT', 'error': 'error'},
        ...
        with self.xm_Give_Object:
            ...
            self.xm_Give_Object.userdata.room_object_pos1=gpsr_target['room_object_pos3']['pos']
            ...
            # 摄像头找,第一个摄像头找不到就第二个,找不到的话爪子打开
            StateMachine.add('TRY_FIND_OBJECT1',
                             TryFindObject1(),
                             transitions={'succeeded': 'FIND_SPEAK1','giveup':'CAN_NOT_FIND_SPEAK1', 'aborted': 'TRY_FIND_OBJECT1','next':'TRY_FIND_OBJECT1','error': 'error'})
            ...
            StateMachine.add('FIND_SPEAK3',
                             FindSpeak(),
                             transitions={'succeeded': 'TRY_FIND_OBJECT4', 'aborted': 'FIND_SPEAK3','error': 'error'})
            StateMachine.add('CAN_NOT_FIND_SPEAK3',
                             SpeakText(),
                             transitions={'succeeded': 'succeeded', 'aborted': 'CAN_NOT_FIND_SPEAK3','error': 'error'},
                             remapping={'sentences':'can_not_find_sentences'})

            StateMachine.add('TRY_FIND_OBJECT4',
                             TryFindObject4(),
                             transitions={'succeeded': 'FIND_SPEAK4','giveup':'CAN_NOT_FIND_SPEAK4', 'aborted': 'TRY_FIND_OBJECT4','next':'TRY_FIND_OBJECT4','error': 'error'})
            StateMachine.add('FIND_SPEAK4',
                             FindSpeak(),
                             transitions={'succeeded': 'succeeded', 'aborted': 'FIND_SPEAK4','error': 'error'})
            StateMachine.add('CAN_NOT_FIND_SPEAK4',
                             SpeakText(),
                             transitions={'succeeded': 'succeeded', 'aborted': 'CAN_NOT_FIND_SPEAK4','error': 'error'},
                             remapping={'sentences':'can_not_find_sentences'})
        ...
        with self.xm_HandMeTHat:
            ...
            # 总任务个数
            self.xm_HandMeTHat.userdata.turn = 4

            self.xm_HandMeTHat.userdata.gettopoint2 = gpsr_target['gettopoint4']['pos']
            self.xm_HandMeTHat.userdata.gettopoint3 = gpsr_target['gettopoint1']['pos']

            self.xm_HandMeTHat.userdata.rec_wait = 6
            ...
            #                 Wait(),
            #                 transitions={'succeeded':'ENTERROOM','error':'error'},
            ...
            StateMachine.add('GO1',
                             NavStack(),
                             transitions={'succeeded': 'FIND_ALL_PEOPLE1', 'aborted': 'aborted', 'error': 'error'},
                             remapping={"pos_xm": 'gettopoint2'})
            
            # 识别到三个人的位置，计算出全局位置并返回
            StateMachine.add('FIND_ALL_PEOPLE1',
                             FindAllPeople2(),  # 识别到三个人的位置，计算出全局位置并返回
                             transitions={'succeeded': 'RECEIVE_TASKS1', 'aborted': 'FIND_ALL_PEOPLE1', 'error': 'error'})
            # 语音问物品,人脸注册
            StateMachine.add('RECEIVE_TASKS1',
                             self.xm_Receive_Task1,
                             transitions={'succeeded': 'GO2',
                                          'aborted': 'RECEIVE_TASKS1', 'error': 'error'})
            #到达第二个
            StateMachine.add('GO2',
                             NavStack(),
                             transitions={'succeeded': 'FIND_ALL_PEOPLE2', 'aborted': 'aborted', 'error': 'error'},
                             remapping={"pos_xm": 'gettopoint3'})
            # 识别到三个人的位置，计算出全局位置并返回
            StateMachine.add('FIND_ALL_PEOPLE2',
                             FindAllPeople2(),  # 识别到三个人的位置，计算出全局位置并返回
                             transitions={'succeeded': 'RECEIVE_TASKS2', 'aborted': 'FIND_ALL_PEOPLE2', 'error': 'error'})
            # 语音问物品,人脸注册
            StateMachine.add('RECEIVE_TASKS2',
                             self.xm_Receive_Task2,

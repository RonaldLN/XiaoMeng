...
class Shopping():
    def __init__(self):
        ...
        with self.GetTask:
            ...
            self.GetTask.userdata.speakbegin = 'Now I will identify items'
            self.GetTask.userdata.find_sentences= 'i find the '
            self.GetTask.userdata.can_not_find_sentences= 'i can not find the object'
            self.GetTask.userdata.current_object_name = ''
            # StateMachine.add('OPEN_GRIPPER',
            #                  GripperCommond(),  # 控制爪子（打开)
            #                  transitions={'succeeded': 'OBJECT_DETECT', 'error': 'error'},
            #                  remapping={'commond':'commond_open'})
            # StateMachine.add("OBJECT_DETECT", TurnDegree_Shop(),
            #                  transitions={'succeeded': 'PRESENT_TASK', 'aborted': 'OBJECT_DETECT', 'error': 'error'})
            
            StateMachine.add('SPEAK_BEGIN',
                             SpeakText(),
                             transitions={'succeeded': 'TRY_FIND_OBJECT1',
                                          'aborted': 'aborted', 'error': 'error'},
                             remapping={'sentences': 'speakbegin'})

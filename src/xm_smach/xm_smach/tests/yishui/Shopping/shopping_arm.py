...
class Shopping():
    def __init__(self):
        ...
        with self.sm_FaceDetect:
            ...
            self.sm_FaceDetect.userdata.sentences = 'Copy that.'
        ...
        with self.DealTask:
            ...
            self.DealTask.userdata.sentence1 = 'please help me get '
            
            StateMachine.add('GET' , Speak_Shopping() , transitions={'succeeded':'CLOSE',
                                                             'aborted':'GET',
                                                             'error':'error'},
                                                    remapping={'sentences':'sentence1',
                                                                'name':'name'
            ...
            self.DealTask.userdata.commond =1
            rospy.sleep(5.0)
            StateMachine.add('CLOSE',
                            GripperCommond(),#控制爪子（bihe）
                            transitions ={'succeeded':'NXT_TASK','error':'error'})
            ...
            #self.DealTask.userdata.take_back_pose_before = [ [0,-1.47,1.67,0,0.24,0] ]
            #self.DealTask.userdata.take_back_pose_after = [ [0,-1.47,3.14,0,0,0] ]
            self.DealTask.userdata.take_back_pose_before = [ [0,-1.47,3.14,0,1.47,0] ]
            self.DealTask.userdata.take_back_pose_after = [ [0,-1.47,3.14,0,0,0] ]
            StateMachine.add('PUT_DOWN', 
                                ArmTrajectory_Point_Before(),
                                transitions={'succeeded': 'ASK_TASK','error': 'error'},
                                remapping={'arm_waypoints':'take_back_pose_before'})

            
            self.DealTask.userdata.sentence2 = 'here you are'
            StateMachine.add('ASK_TASK' , Speak() , transitions={'succeeded':'OPEN',
                                                             'aborted':'ASK_TASK',
                                                             'error':'error'},
                                                    remapping={'sentences':'sentence2'})
            self.DealTask.userdata.commond =0
            rospy.sleep(3)

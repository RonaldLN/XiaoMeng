...
from smach_special.StoringGroceries_lib import * 
...
Author: yishui
Date: 2022-08-25
LastEditTime: 2022-08-25 13:49:18
LastEditors: yishui
...
class StoringGroceries():
    def __init__(self):
        ...
        with self.xm_StoringGroceries:
            # 设定sort=1为食物，sort=2为水果，sort=3为其他
            self.xm_StoringGroceries.userdata.sort=1
            self.xm_StoringGroceries.userdata.current_turn = 0
            ...
            self.xm_StoringGroceries.userdata.target_food = 'food'
            self.xm_StoringGroceries.userdata.target_drinks = 'drinks'
            self.xm_StoringGroceries.userdata.target_others = 'toiletries'

            StateMachine.add('PICK_UP_FOOD',
                                self.xm_Pick_up,
                                remapping ={'target':'target_food'},
                                transitions={'succeeded':'PUT_DOWN_FOOD','aborted':'CHECK_SORT','error':'CHECK_SORT'})
            ...
            StateMachine.add('PICK_UP_DRINKS',
                                self.xm_Pick_up,
                                remapping ={'target':'target_drinks'},
                                transitions={'succeeded':'PUT_DOWN_DRINKS','aborted':'CHECK_SORT','error':'CHECK_SORT'})

            StateMachine.add('PUT_DOWN_DRINKS',
                                self.xm_Put_down,
                                remapping ={'target':'target_drinks'},
                                transitions={'succeeded':'CHECK_TURN','aborted':'PUT_DOWN_DRINKS'}) 

            StateMachine.add('PICK_UP_OTHERS',
                                self.xm_Pick_up,
                                remapping ={'target':'target_others'},
                                transitions={'succeeded':'PUT_DOWN_OTHERS','aborted':'GO_OUT','error':'GO_OUT'})
            ...
            StateMachine.add('CHECK_SORT',
                                CheckSort(),
                                transitions={'drinks':'PICK_UP_DRINKS','others':'PICK_UP_OTHERS','error':'error'})   

            StateMachine.add('CHECK_TURN',
                                CheckTurn(),
                                transitions = {'succeeded':'GO_OUT','food':'PICK_UP_FOOD', 'drinks':'PICK_UP_DRINKS',
                                 'others':'PICK_UP_OTHERS','error':'error'}
            )

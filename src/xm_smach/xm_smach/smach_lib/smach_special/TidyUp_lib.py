...
class GetTargetPosition(State):
    ...
    def execute(self, userdata):
        ...
        #     rospy.sleep(2.0)
            a = subprocess.Popen(['python3','/home/xm/catkin_ws/src/xm_vision/src/scripts/Tidy/vison_server.py','-d','true'] ,shell =False)
...
class GetPutPosition(State):
    ...
    def execute(self, userdata):
        ...
        #     subprocess.call("xterm -e python3 /home/xm/catkin_ws/src/xm_vision/src/scripts/mrsupw_detect_object.py",shell=True)
        #     rospy.sleep(2.0)
            a = subprocess.Popen(['python3','/home/xm/catkin_ws/src/xm_vision/src/scripts/Shopping/backetlocation.py','-d','true'] ,shell =False)
            with open("/home/xm/vision_pid/people_tracking.txt",'w+') as f:
                if f.read() == '':
                    print('-------------------')
                    print('pid',a.pid)
        ...
        try:
            ...
            if res.object.pos.point.x != 999 or res.object.pos.point.y != 999 or res.object.pos.point.z != 999:
                rospy.loginfo("find object!")
        except Exception, e:
            rospy.logerr(e)
            rospy.logwarn('bad call the service')
...
class NextDo(State):###跳转action
    def __init__(self):
        State.__init__(self, 
                    ...
                    outcomes=['succeeded','aborted','follow','get','error'],
    ...
    def execute(self, userdata):
        ...
        if current_action == 'follow':
            return 'follow'
        # shopping中如果没有动词的话，返回remember
        elif current_action == 'pick' or current_action == 'get' or current_action == 'take' or current_action == 'remember':
            return 'get'
...
class GetPickPosBack_tidyup(State):
    '''
    no use
    根据图像传过来的数据，确定抓取时xm的位置
    up_down变量为1，则使用上面的摄像头；up_down变量为0，则使用下面的摄像头。

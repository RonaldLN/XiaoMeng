...
class OpenDoorDetect(State):
    ...
    def execute(self, userdata):
        try:
            a = subprocess.Popen(
                ['python3', '/home/xm/catkin_ws/src/xm_vision/src/scripts/GPSR/Gate.py', '-d', 'true'], shell=False)
            with open("/home/xm/vision_pid/people_tracking.txt", 'w+') as f:
                if f.read() == '':
                    print('-------------------')
                    print('pid', a.pid)
    ...
    def __init__(self):
        State.__init__(self,
                       ...
                       io_keys=['people_pos', 'current_people_name',
                                'current_people_pos'],
                       input_keys=['current_guest_num', 'people_name', 'target', 'turn', 'name_pos_dict', 'pos_gone_bool'])
...
class FindObjectCount(State):
    def __init__(self):
        State.__init__(self,
                       outcomes=['succeeded', 'finish','error'],
                       io_keys=['degree', 'find_object_count'])

    def execute(self, userdata):
        
        try:
            object_count= userdata.find_object_count

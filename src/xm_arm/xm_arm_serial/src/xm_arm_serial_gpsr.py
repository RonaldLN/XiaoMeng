...
class ARM_SERIAL:
    ...
    def __init__(self):
        ...
        portx_arm = "/dev/ttyUSB2"
        # portx_base = "/dev/ttyUSB0"
        ...
        timex = 50
        # 打开串口，并得到串口对象
        self.ser_arm = serial.Serial(portx_arm, bps, timeout=timex)
        # self.ser_base = serial.Serial(portx_base, bps_base, timeout=timex)
        #检测串口是否打开（这里应该用异常处理！！）
        if (self.ser_arm.isOpen()):
...
#与串口建立通信(shaking_Hands)
        ...
        self.ser_arm.write(shaking_Hands)
        # self.ser_base.write(shaking_Hands)
        time.sleep(0.1)
        ...
        #初始化机器人位姿
        # self.arm_pos = [0,2.617996938958333,-4.0274969389583335,-0.004999999999999893]
        self.arm_pos = [0,-1.06,-2.20,-1.12]

        self.arm_pos_rightOrder = [0,0,0,0]
        ...
        self.joint_names = [
            'lift_joint','arm_joint_0', 'arm_joint_1', 'arm_joint_2','arm_joint_3','left_gripper_joint'
        ]
        ...
        self.cmd_vel = rospy.Publisher('/plat_control', plat_control, queue_size=1)
        twist_xm = plat_control()
        twist_xm.height = self.plat_pos[0]
        for i in range(10):
            self.cmd_vel.publish(twist_xm)
            time.sleep(0.1)
        ...
        def floatToBytes(f):
            bs = struct.pack("f", f)
            list1 = list()
            list1 = [bs[0:1], bs[1:2], bs[2:3], bs[3:4]]
            return list1
...
#设置发送频率
        ...
        while not rospy.is_shutdown():
            ...
            # print(plat_pos_serial)
            # self.ser_base.write(plat_pos_serial)
            #打包arm
            arm_pos_serial = [
                '\xff', '\xff', '\x01', '\x11', '\x00', '\x0D', '\x03'
            ]
            # arm_pos_rightOrder = [0,0,0,0,0,0]
            self.arm_pos_rightOrder[0] = self.arm_pos[0]
            self.arm_pos_rightOrder[1] = self.arm_pos[1]
            self.arm_pos_rightOrder[2] = self.arm_pos[2]
            for pos in self.arm_pos_rightOrder[0:3]:
            ...
            # print('arm_pos_serial',arm_pos_serial)
            # print(arm_pos_serial)
            self.ser_arm.write(arm_pos_serial)
            #打包gripper
            gripper_pos_serial = [
                '\xff', '\xff', '\x01', '\x11', '\x00', '\x03', '\x01'
            ]
            gripper_pos_serial += self.gripper_pos[0]  #柔性爪
            # print("self.gripper_pos[0]",self.gripper_pos[0])
            gripper_pos_serial += self.gripper_pos[0]  #dora'
            ...
            # print('gripper_pos_serial',gripper_pos_serial)
            #gripper_pos_serial += ['\x17']
            self.ser_arm.write(gripper_pos_serial)
            ...
            self.joint_states.position = list(self.plat_pos) + list(self.arm_pos) + list(self.gripper_pos_1) 
    ...
    def set_plat_pos(self, req):
        twist_xm = plat_control()
        twist_xm.height = -req.height
        print('plat height:',twist_xm.height)
        for i in range(10):
            self.cmd_vel.publish(twist_xm)
            time.sleep(0.1)
        # time.sleep(10)

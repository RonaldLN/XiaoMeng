...
class ARM_SERIAL:
	...
	PI = 3.14
        ...
        #初始化机器人位姿
        self.arm_pos = [-0.05,-1.4,3.14,0,1.71,-1.47]	
	...
	self.gripper_pos_1 = [1]
        ...
        self.joint_names = ['lifting_joint', 'arm_joint_0', 'arm_joint_1', 'arm_joint_2', 'arm_joint_3', 'arm_joint_4', 'arm_joint_5'
                            ,'active_finger_joint', 'camera_joint_0', 'camera_joint_1']
	...
	#设置发送频率
        ...
        while not rospy.is_shutdown():
            ...
            #将关节信息整理成串口数据包
	    #打包plat
	    plat_pos_serial = ['\xff','\xff','\x01','\x11','\x00','\x05','\x05']
	    plat_pos_serial += floatToBytes(self.plat_pos[0])
	    plat_pos_check = ['\x00']
	    for num in plat_pos_serial:
		plat_pos_check[0] = chr((ord(plat_pos_check[0]) + ord(num))%255)
	    plat_pos_serial += plat_pos_check
	    self.ser.write(plat_pos_serial)
	    #打包arm
	    arm_pos_serial = ['\xff','\xff','\x01','\x11','\x00','\x19','\x03']
		...
		arm_pos_check[0] = chr((ord(arm_pos_check[0]) + ord(num))%255)
	    arm_pos_serial += arm_pos_check
	    self.ser.write(arm_pos_serial)
	    #打包gripper
	    gripper_pos_serial = ['\xff','\xff','\x01','\x11','\x00','\x03','\x01']
	    ...
	    #从串口读出实际的关节角度，并发布到/joint_states话题中（这里并没有从串口读）   
            self.joint_states.position = list(self.plat_pos) + list(self.arm_pos) + list(self.gripper_pos_1) + list(self.camera_pos)

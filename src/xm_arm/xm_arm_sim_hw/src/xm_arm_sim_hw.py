...
class ARM_SIM_HW:
    ...
    def __init__(self):
        ...
        # self.server = actionlib.SimpleActionServer('/xm_arm/arm_stack', xm_ArmStackAction)
        # self.server.start()

        #为了仿真的效果，我们在这里设置了关节更新的速度
        # self.plat_velocity = 0.04  
        self.plat_velocity = 0.08                                             #升降移动的速度             m/s
        ...
        #初始化机器人位姿
        # self.arm_pos =[0,0.3200, 0.3000, 0]
        ...
        while not rospy.is_shutdown():
            ...
            #总和各个关节位置并发出
            self.joint_states.position = list(self.plat_pos) + list(self.arm_pos)
            pub.publish(self.joint_states)

            
            # rospy.loginfo("""\033[1;30;47m
            # amr_position:
            #     arm_pose:
            #         %f
            #         %f
            ...
            #     plat_pose:
            #         %f
            # \033[m""",self.arm_pos[0],self.arm_pos[1],self.arm_pos[2],self.plat_pos[0])
    ...
    def set_arm_pos(self,data):
        ...
        更新机械臂关节position
        '''
        # arm_direction = [0,0.3200, 0.3000, 0]
        arm_direction = [0,0,0,0]
        count = [0,0,0,0]
        # tmp_arm_pos = [0,0.3200, 0.3000, 0]
        tmp_arm_pos = [0,0,0,0]

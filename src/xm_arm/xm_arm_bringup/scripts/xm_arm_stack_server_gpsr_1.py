...
pick_pose_2 = [0,-0.615,3.14,0,0,0]
...
class ArmStack:
	...
	#ready to compute
	plat_goal_1 = get_gripper_or_plat_goal_from_position(-0.20)#这个高度是为了防止挡着摄像头
	self.plat_client.send_goal(plat_goal_1)
	self.plat_client.wait_for_result(rospy.Duration(60.0))
	rospy.loginfo("...plat down")
	  
	ready_pick_trajectory = get_singel_pos_trajectory(pick_pose_1,self.joint_names)
	...
	rospy.sleep(5)
	    ...
	    target_arm_joint_0_point.point.x = target_arm_joint_0_point.point.x-0.01
	    target_arm_joint_0_point.point.y = target_arm_joint_0_point.point.y+0.11
	    arm_length,arm_joint_0_angle = self.get_arm_length_and_arm_joint_0_angle(target_arm_joint_0_point)
	    arm_solution,arm_solution_is_succeed = self.get_arm_solution(arm_length,target_arm_joint_0_point,arm_joint_0_angle,target_base_pose.point.z+0.02)
	    ...
	    if arm_solution_is_succeed == True :
		    ...
		    rospy.sleep(2)
		    back_pose = arm_solution_angle
		    back_pose[4] = -0.1 
		    back_trajectory = get_singel_pos_trajectory(back_pose,self.joint_names)
		    back_goal = get_goal_from_trajectory(back_trajectory)
		    self.arm_client.send_goal(back_goal)
		    self.arm_client.wait_for_result(rospy.Duration(60.0))
		    rospy.sleep(3)
		    back_pose_2 = [0,-0.615,3.14,0,0,0]
		    back_pose_2[0] = arm_solution_angle[0] 
		    rospy.loginfo(back_pose_2)
		    back_trajectory_1 = get_singel_pos_trajectory(back_pose_2,self.joint_names)
		    back_goal_1 = get_goal_from_trajectory(back_trajectory_1)
		    self.arm_client.send_goal(back_goal_1)
		    self.arm_client.wait_for_result(rospy.Duration(60.0))
		    rospy.sleep(5)
		    ready_pick_trajectory_1 = get_singel_pos_trajectory(pick_pose_2,self.joint_names)
		    rospy.loginfo(pick_pose_2)
		    ready_pick_goal_1 = get_goal_from_trajectory(ready_pick_trajectory_1)
		    self.arm_client.send_goal(ready_pick_goal_1)
		    self.arm_client.wait_for_result(rospy.Duration(60.0))

...
public:
	boost::shared_ptr<boost::asio::serial_port> port_;//一个指向boost::asio::serial_port类型的智能指针，串口通信由asio组件的serial_port类完成
	...
	void readHandler(const boost::system::error_code &ec, size_t bytesTransferred);//被start_a_read调用处理读取
	void writeHandler(const boost::system::error_code &ec);//被start_a_write调用处理写入
	int read_count;
};
}

...
{//分析包的完整性，完整时分析包并执行所属操作
    ...
    if( receive_message_update ==1 )//假如收到了一个效验通过的包
    {
        receive_message_update = 0;
        package_update=packageAnalysis();
        /*对数据包进行分析，只要包没问题返回都是1*/
...
该状态机呈瀑布状，一步步向下执行，前一个状态为后面所有状态的前置条件，如果出现问题就回到FF1级等待状态*/
    ...
    case WAITING_FF1:
        if (rx_data == 0xff)
        {/*先将receive_check_sum置0，再加上目前的收到的数据rx_data,作为日后效验的凭据
    ...
    /*xiao此时rx_data就是sender_id*/
        ...
        //ROS_INFO_STREAM(friend_id);
        
        /*
        if(rx_message_.sender_id==0x01)
            printf("sender_id is 0x01 \n");
            
        if(rx_message_.sender_id==0x11)
            printf("sender_id is 0x11 \n");
        if(friend_id==0x01)
            printf("friend_id is 0x01 \n");
        if(friend_id==0x11)
            printf("friend_id is 0x11 \n");
        */
    ...
    case RECEIVE_LEN_H:
    //此时rx_data是数据长度高8位的信息，因封装上传时将它右移了8位，现在恢复一下
        receive_check_sum_ += rx_data;
    ...
    case RECEIVE_LEN_L:
    //现在rx_data是数据长度低8位的信息，与之前的数据相或可以恢复成正确的数据长度
        receive_check_sum_ += rx_data;
    ...
    //只有收到了包才会将信息记载进data
        ...
        {//为确保整个报的数据被读入，只有当byte_count_>= receive_message_length_才会跳转向RECEIVE_CHECK
...
{/*分析完协议后，对从上位机和下位机间的交互命令进行分析，返回值有两种，1和0*/
    ...
    {                                //此处握手指的应是数据传输之间的协议，表示开始数据传输前的交互
    ...
    {//处理命令，大致可以将处理分为设定数据（setCommandAnalysis）和上传数据（readCommandAnalysis）两个函数。
    case SHAKING_HANDS :
    ...
    */
    /*
    case SET_SOUND_START:
        analysis_state = setCommandAnalysis(command_state_, (unsigned char *)&my_robot->sound_start, sizeof(my_robot->sound_start));
        break;

    case READ_SOUNDANGLE:
        analysis_state = readCommandAnalysis(command_state_, (unsigned char *)&my_robot->sound_angle, sizeof(my_robot->sound_angle));
        break;

    case READ_ULTRASOUND:
        analysis_state=readCommandAnalysis(command_state_, (unsigned char *)&my_robot->ultra_sound[15], sizeof(my_robot->ultra_sound[15]));
        break;
   */
   // case SET_NECK://10.26修改
     //   ;
    ...
    return analysis_state;//只要包没问题,且假如收到CLEAR_COORDINATE_DATA时本机是下位机，此处返回都是1
}
//执行读取命令
...
{    
    ...
    if (hf_link_node_model==1)//上位机收到读取的命令后，自动更新机器人的相关部件信息
    { // master   , means the slave feedback a package to master , and the master save this package
        if((rx_message_.length-1) != len)//确认下位机上传的信息长度与机器人相关部件信息长度是否匹配
        {
           // printf("I'm a master , can not read the message from slave , the length is not mathcing to struct \n");
            return 0;
        }
        memcpy(p, &rx_message_.data[1] , len);//用接收的命令更新机器人相关部件信息
        receive_package_renew[(unsigned char)command_state] = 1 ;//更新相关接收标志
    }
    else if(hf_link_node_model==0)//下位机收到读取命令后，自动上传相应的机器人部件信息
    { // slave   , means the master pub a read command to slave ,and the slave feedback the a specific info to him
        ...
        //上位机要求读取机器人相关部件信息，下位机包装好后发送回去，此时不用再管接收到的数据（rx_message）,所以不用效验
...
{//第一个参数是接收到的命令，第二个参数是命令描述的相关机器人部件，第三个参数是该部件数据结构大小
    void* ack;//这个ack对于上、下位机通信和对于电子组在调试晓萌时都是挺有用的，可以看到晓萌是否收到了对应的指令
    ...
    { // master  , the slave can set the master's data ,so this code means received the slave's ack
        if(command_state==SHAKING_HANDS) {
            shaking_hands_state=1;  //wait he master send SHAKING_HANDS假如接收的命令是请求握手，那么握手成功
         //   printf("received a SHAKING_HANDS commmand and the slave is waiting master send SHAKING_HANDS data ");
        }
        else//上位机收到设置命令后，明白下位机已正常执行设置命令，确认收到ack，对于上位机会把除了SHAKING_HANDS外的所有设置命令当做ack
        {
            //ack Analysis
          //  printf("received a ack ");
        }
        receive_package_renew[(unsigned char)command_state] = 1 ;//更新接收标志
    }
    else if(hf_link_node_model==0)//下位机收到设置命令后，根据设置命令更新相应的机器人描述数据，即通过调整参数来控制运动，以此响应命令
    { // slave  , means the master pub a set command to slave ,and the slave save this package then feed back a ack
        if((rx_message_.length-1) != len)//确定机器部件数据类型大小是否与收到的数据大小匹配
        {
            printf("I'm a slave , can not read the message from master , the length is not mathcing \n");
            return 0;
        }
        //从rx_message_.data[1]复制len个字节到p，p即机器人相关部件，此即为更新机器人相关数据
        memcpy(p , &rx_message_.data[1] , len);//就算是握手命令，这一步也会被执行，更新测量的全局坐标系
...
{///更新机器人相关部件数据  这一部分是上位机对下位机发送指令，电子组掌握的代码中这个该函数并没有什么用

    //else  means master send a command to slave
    unsigned char analysis_state =1;
    void* single_command;
    if(hf_link_node_model != 1){return 0;}//假如是下位机就直接退出，防止误调
...
/*
   case SET_SOUND_START:
        receivekkage_renew[(unsigned char)command_state] = 0;
        //std::cout<<"SET_SOUND_START"<<std::endl;
        sendStruct(command_state, (unsigned char *)&my_robot->sound_start, sizeof(my_robot->sound_start));
        break;
*/
    }

    return analysis_state;//假如传回为0，则说明命令种类未知
}
...
void HFLink::sendStruct(const Command command_state , unsigned char* p , const unsigned short int len)//组装所要发送的tx_message_，字符串级别
{
    ...
    if (len == 0)
    {
        //如果len是0，则说明这是下位机发出的请求握手，此时数据区里没数据
    }
    else if(len > 0)
    {
        memcpy(&tx_message_.data[1] , p , len);//从p所指位置拷贝len个字节到tx_message_.data[1]
    }
    sendMessage();
}
...
{//将封装包的东西导入上传区
    ...
    //发送缓冲区一次含有一个包，每次组装包会更新发送缓冲区,前两个信息都是0xff
    tx_buffer[0]=0xff;
    check_sum_ += 0xff;
    ...
    tx_buffer[2]=tx_message_.sender_id;//发送者id
    check_sum_ += tx_buffer[2];

    tx_buffer[3]=tx_message_.receiver_id;//接受者id

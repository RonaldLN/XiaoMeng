...
void TransportSerial::mainRun()//重复从底层读取data型数据包
{
    ...
    std::cout << "Transport main read/started" <<std::endl;
    start_a_read();
    ios_->run();//：阻塞执行事件循环
}
...
void TransportSerial::start_a_read()
{
    ...
    port_->async_read_some(boost::asio::buffer(temp_read_buf_),//此处buffer为boost库的函数，一般参数都要buffer一下，该参数是一个临时异步阅读缓冲区
...
void TransportSerial::readHandler(const boost::system::error_code &ec, size_t bytesTransferred)
{
    ...
    boost::mutex::scoped_lock lock(read_mutex_);//锁定读取
        Buffer data(temp_read_buf_.begin(), temp_read_buf_.begin() + bytesTransferred);//从临时异步阅读缓冲区读取数据
...
void TransportSerial::start_a_write()
{
    ...
    if (!write_buffer_.empty())//判读写缓冲区是否为空，如果不为空则从写缓冲区将数据写入端口，并弹出
    {
...
void TransportSerial::writeHandler(const boost::system::error_code &ec)
{
    if (ec)
    {
        std::cerr << "Transport Serial Error "<< std::endl;
        return;
    }

    boost::mutex::scoped_lock lock(write_mutex_);//锁定写入
    if (!write_buffer_.empty())	start_a_write(); //只要写入缓冲区不为空就继续写入
}

//从缓冲区读取数据并返回
Buffer TransportSerial::readBuffer()
{
    //智能锁在该类对象构造时自动调用相应方法，锁住资源，该语句块终止时会自动调用其析构函数，释放资源
    boost::mutex::scoped_lock lock(read_mutex_);//锁定读取

    if (!read_buffer_.empty())//假如读取缓冲区有数据就拷贝第一个元素，并且将其从缓冲区弹出
    {
...
bool TransportSerial::initializeSerial()
{
    try//初始化串口的传输参数
    {
        ...
        //利用串口类初始化串口
        port_->set_option(
                    boost::asio::serial_port::baud_rate(params_.baudRate));//波特率
        port_->set_option(
                    boost::asio::serial_port::flow_control((boost::asio::serial_port::flow_control::type)params_.flowControl));//流量控制
        port_->set_option(
                    boost::asio::serial_port::parity((boost::asio::serial_port::parity::type)params_.parity));//奇偶校验
        port_->set_option(
                    boost::asio::serial_port::stop_bits((boost::asio::serial_port::stop_bits::type)params_.stopBits));//结束位
    ...
    catch(std::exception &e)
    {
        ...
        initialize_ok_ = false;//新加入
        return false;
    }

    //以下两行为查看源码后新加入，判断初始化已经成功

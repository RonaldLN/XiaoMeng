...
public:
	...
	inline boost::shared_ptr<boost::asio::io_service> getIOinstace()//返回io_service对象指针
	{
	...
	inline bool initialize_ok () const//const为只读属性
	{
...
private:
	...
	boost::shared_ptr<boost::asio::deadline_timer> timer_;	//维护一个时间，即线程的阻塞等待
	...
	inline uint8_t checkUpdate(const Command command_state)//是否已发送命令标志&是否接受命令标志   如果发送了相关命令，而却没有接受相关命令，返回就为0
	{

...
{                
    ...
    std::string transport_method = url.substr(0, url.find("://"));//复制"://"前的字符串
    if (transport_method == "serial")//如果串口成功接入
    {                   
        port_ = boost::make_shared<TransportSerial>(url); //初始化基类串口 make_shared消除new调用，创建指针对象并返回
        time_out_ = 20;//真的是这个值导致底盘卡吗，对确实是   原值100，handsfree开源社区用的是500,后来改为20
        hflink_ = boost::make_shared<HFLink>(0x01, 0x11, &my_robot_);
      
        timer_.reset(new boost::asio::deadline_timer(*(port_->getIOinstace()),//第一个参数是一个io_service对象指针
        boost::posix_time::milliseconds(time_out_))); //构建一个以毫秒为单位的时间长度，第二个参数是等待的绝对时间
    ...
    file_.open(config_addr.c_str(), std::fstream::in);//文件输入流，输入config_addr文件
    if (file_.is_open())
    {
    ...
    if(use_sim_ == 0)
    {
        try
        {
            thread_2 = boost::thread(boost::bind(&HF_HW::updateReadCommand, this));//bind只是加载，构造线程对象(&HF_HW::updateReadCommand（this）
        }
        catch(std::exception &e)
        {
            std::cerr << "read thread create failed " << std::endl;
            std::cerr << "Error Info: " << e.what() <<std::endl;//what提取异常信息，打印出一串文字
        }
    }
}
...
void HF_HW::timeoutHandler(const boost::system::error_code &ec)
{
    if (!ec)
    {
        ...
        boost::mutex::scoped_lock lock(wait_mutex_);//互斥锁，多线程并发执行时对互斥资源线程安全的访问
        time_out_flag_ = true;
    }
}
...
void HF_HW::updateRobot()
{
    ...
    memset(hflink_count_, 0 ,sizeof hflink_count_); //将hflink_count_用0填充
    int count = 0;//用来控制命令发送频率
    while (true)
    {
        cicle_timer_.expires_from_now(boost::posix_time::millisec(30));//100hz 设置10毫秒得程序运行时间,改为30试试
        std::cout<< "start a write" <<std::endl;
        //  check hand shake
        checkHandshake();
        //  uodate normal data：将hflink_command_set_current_用0填充
        memset(hflink_command_set_current_, 0, sizeof hflink_command_set_current_);//将hflink_command_set_current_用0填充
        for (int i = 1; i < LAST_COMMAND_FLAG; i++)//该循环会把所有需要发的命令发送一遍
        {
            if (hflink_command_set_[i] != 0)//不为0意味着这个命令会被发送
            {
                int cnt = count % 100;//controller_freq_ =  100,原来为100，据计算猜测此值为cotroller_freq,发现无法编译，可能是只有一些launch文件包含cotroller_freq
                if (cnt %  (100 / hflink_freq_[i]) == 0)//控制各命令发送频率
                {
                    sendCommand((Command)i);
                    hflink_command_set_current_[i] = 1;//表示对应命令已经被发送
                }
            }
        }
        ...
        ack_ready_ = false;
        while (!ack_ready_)//超时或者发送的所有命令下位机都正常接受了才会退出循环
        {
            for (int i = 0; i < data.size(); i++)//该循环将把整个数据包分析一遍
            {
                ...
                //只要收到了包且包没问题就为true。在整个数据包的分析过程中，只有分析到最后的效验阶段，并且通过了才会执行下面代码块
                {
                    ...
                    if(!temp)
                    {
                        std::cerr<<"don't resieve data"<<std::endl;
                    }
                }
            }
            if (cicle_timer_.expires_from_now().is_negative())//执行超时处理
            {
...
bool HF_HW::updateWriteCommand(const Command &command, int count)//按照相应频率发送命令
{
    if (hflink_command_set_[command] != 0)
    {
        int cnt = count % 100;//controller_freq_ =  100,原来为100，据计算猜测此值为cotroller_freq_
        ...
        } else
        {
            // skip this package
            return false;
            std::cerr<<"can't  write to serial"<<std::endl;
        }
    }
    //上面的代码完成定时向stm32发送指令，下面的代码完成分析接收包的信息
    // 相当于每发送一次指令后,程序就卡在下述代码段等待数据反馈
    //std:: cerr<<"write all ready"<<std::endl;
    return true;
}
...
void HF_HW::updateReadCommand()
{
    while(1)
    {
            ...
            boost::asio::deadline_timer t(io, boost::posix_time::millisec(15));  //从现在开始15ms后终止的定时器
            t.wait();  
           ...
           /*for(int i=0;i<data.size();i++)
            ...
            if(!data.empty())
            {
                ...
                ack_ready_ = false;
                bool flag = 0;
                while (!ack_ready_)//：两种情况下会退出循环，一是超时，二是下位机成功接受了命令
                {
                    // for (int i = 0; i < data.size(); i++)
                    // {注释于1.27
                    ...
                    for (int i = 0; i < data.size(); i++)//假如data为空，则size()运行结果为0
                    {
                        if (hflink_->byteAnalysisCall(data[i]))//分析包的完整性，完整时分析包并执行所属操作,对机器人ADT的变量更新即在这层调用完成
                        {
                            ...
                            //std::cerr<<"I complete a package command is: "<<std::endl;
                            ack_ready_ = true;
                        }//else{

...
room={
    ...
    'cookingbench':{'center':[-4.3474, -6.5712],'size':[1.0,2.0]},
    'kitchentable':{'center':[-4.3432,-7.7959],'size':[1.0,1.0]},
    'fridge':{'center':[-4.1416,-9.9343],'size':[0.6,0.6]},
    'kitchencounter':{'center':[-0.52968,-9.0139],'size':[0.6,0.6]},
...
class FixInformation(State):
    ...
    def get_location(self, pos):
        ...
        for key in room:
            print(key)
            size_a = abs(position.position.x - room[key]['center'][0])
            size_b = abs(position.position.y - room[key]['center'][1])
            print(size_a)
            print(size_b)
            if size_a * size_a + size_b * size_b < min_percent:
...
class FindMyMates():
    def __init__(self):
        ...
        with self.FindMyMates:
            ...
            # 操作员位置
            self.FindMyMates.userdata.start_pos = gpsr_target['kitchen']['pos']
            # 观察位置
            self.FindMyMates.userdata.scan_pos = gpsr_target['kitchen']['pos']
            # 退场位置
            self.FindMyMates.userdata.exit_pos = gpsr_target['kitchen']['pos']
            ...
            self.FindMyMates.userdata.name_list = ['Alex','John','Mary']
